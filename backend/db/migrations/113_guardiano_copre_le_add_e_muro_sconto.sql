-- =====================================================================
-- 113 — Il guardiano copre anche le ADD, e muro/sconto perdono il PC
-- =====================================================================
-- BUCO MISURATO 10/09 21:55: il feed che va a Farmabooster (stable_price_cuts)
-- non contiene solo i PRICE_CUT: contiene anche le azioni ADD che portano un
-- recommended_price (externalApi.js: "fa.action = 'PRICE_CUT' OR fa.action =
-- 'ADD'"). Il guardiano guardava solo i PRICE_CUT.
-- Misura sugli snapshot rigenerati: 711 PRICE_CUT nel feed, ZERO sotto floor.
-- 144 ADD con prezzo, 141 SOTTO FLOOR e 22 SOTTO COSTO (MPF 70, Procaccini 52,
-- Farmainsieme 11, Papa 7, Farmastelia 1). Erano prezzi AI vivi sotto floor
-- passati da una porta che nessuna guardia sorvegliava.
--
-- Ordine capo 10/09 #6: "non vengono emessi pc per prodotti che sono in sconto
--   o in muro. e se un prodotto non era in muro e aveva un PC nel momento in
--   cui entra nella regola muro il pc viene annullato".
--
-- Questa migrazione:
--   1. il guardiano legge PRICE_CUT + ADD-con-prezzo;
--   2. una ADD non si cancella mai (butterebbe il prodotto fuori dal feed):
--      quando il prezzo non e' riparabile si toglie il PREZZO;
--   3. muro/sconto: il PRICE_CUT viene annullato, la ADD resta senza prezzo.
-- =====================================================================
...
SELECT '113_guardiano_copre_le_add_e_muro_sconto.sql', NOW()
WHERE NOT EXISTS (SELECT 1 FROM schema_migrations WHERE filename = '113_guardiano_copre_le_add_e_muro_sconto.sql');
