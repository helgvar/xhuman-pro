-- 136 — Il guardiano PC deve coprire OGNI tenant che espone un feed.
--
-- Difetto trovato il 15/09/2026: reconfirm_price_cuts_v2 limitava il controllo
-- a v_op (8 tenant), lasciando fuori San Vito e Ospedale. Ma quei due DISPATCHANO
-- (San Vito 36.680 civetta=1, Ospedale 5.585): i loro prezzi sono esposti come
-- tutti gli altri. Misura del giorno: 83 PC sotto il floor di fascia sulla rete,
-- di cui 40 proprio su San Vito (32) e Ospedale (8), che da soli pesano 15,10
-- dei 17,22 euro fuori floor. Gli altri 43, dentro il perimetro, avevano scarti
-- da 0,02 a 0,17% — arrotondamento: il guardiano li stava gia' tenendo.
--
-- Legge capo 10/09: "per niente al mondo i prezzi ai possono andare sotto floor.
-- non esiste nessun veto o nessun ordine manuale che puo' bloccare il ricalcolo
-- di un prezzo ai che va sotto floor". "Non operativo" non e' un veto ammesso:
-- se un tenant non espone nulla, il perimetro resta vuoto da solo e non costa
-- niente; se espone, va controllato.
--
-- Unica modifica: v_op passa da 8 a 10 tenant. Nessun'altra riga toccata.
...
DECLARE
  v_op   text[] := ARRAY['Papa','Farmacia Procaccini','MPF','Farmainsieme',
                         ...
                         'Farmacri','Farmacia San Vito','Farmacia Ospedale'];  -- mig 136: ogni tenant che espone un feed
...
VALUES ('136_guardiano_pc_copre_tutta_la_rete.sql', NOW())
